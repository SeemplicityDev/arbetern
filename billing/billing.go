...
func New(ctx context.Context, b *store.Backend) (*Store, error) {
	...
	if n := s.Months(); n > 0 {
		log.Printf("[billing] loaded %d month(s) of usage from %s", n, b)
	}
	return s, nil
}
...
func (s *Store) Refresh(ctx context.Context) error {
	...
	// events put back: every other cached month still carries them from when
...
func (s *Store) loadRecent(ctx context.Context) ([]Event, error) {
...
func (s *Store) StartRefresh(ctx context.Context, interval time.Duration) {
	safego.Tick(ctx, "billing: refresh", interval, func() {
		if err := s.Refresh(ctx); err != nil {
			log.Printf("[billing] refresh: %v", err)
		}
	})
}
