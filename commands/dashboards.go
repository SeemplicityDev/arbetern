					...
					"properties":{
						...
						"sources":{
							...
							"items":{
								"type":"object",
								"properties":{
									"type":{"type":"string","enum":["jira_search","salesforce_query","chorus_list_conversations","datadog_search_logs","datadog_list_monitors","datadog_query_metrics","confluence_search","github_list_prs"],"description":"Which integration to call. Each type accepts a specific 'args' shape — see below. IMPORTANT: when the user asks about numeric metrics over time (CPU, memory, latency, throughput, error rate, pod count, request rate, p95/p99, etc.) use 'datadog_query_metrics' — NOT 'datadog_list_monitors'. 'datadog_list_monitors' only lists alert definitions, it never returns the actual metric values or filters services by their usage."},
									"name":{"type":"string","description":"Short label for this source, shown as the section title on the rendered dashboard."},
									"args":{
										"type":"object",
										"description":"Type-specific arguments. jira_search: {jql: string, max_results?: int}. salesforce_query: {soql: string}. chorus_list_conversations: {min_date?, max_date?, engagement_type?, content_type?, participants_email?, user_id?, team_id?, engagement_id?, with_trackers?:bool}. datadog_search_logs: {query: string, site?: 'us'|'eu', from?: string, to?: string, limit?: int}. datadog_list_monitors: {site?: 'us'|'eu', query?: string, limit?: int}. datadog_query_metrics (renders as a timeseries line chart in the dashboard UI): {query: string — a full Datadog metric query like 'avg:kubernetes.cpu.user.total{env:prod} by {kube_service}', site?: 'us'|'eu', from?: string (default '-1h'; accepts '-1h', '-24h', ISO-8601, or unix seconds), to?: string (default now), top_n?: int (default 20, max 100; number of series to keep, sorted by avg desc)}. confluence_search: {cql: string, limit?: int}. github_list_prs: {repo: string, state?: 'open'|'closed'|'all', limit?: int}."
									}
								},
